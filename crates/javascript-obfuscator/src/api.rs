...
mod tests {
    ...
    #[test]
    fn obfuscate_transform_object_keys_simple_variable_declarator_when_enabled() {
        let result = obfuscate(
        ...
        assert!(result.code.contains("var object=_0x0;"), "{}", result.code);
    }
    ...
    fn obfuscate_renames_properties_in_unsafe_mode() {
        ...
        assert!(
            result.code.contains("const object={'_0x0':0x1};"),
            "{}",
            result.code
        );
        assert!(result.code.contains("object._0x0"), "{}", result.code);
    ...
    fn obfuscate_renames_top_level_destructuring_properties_in_unsafe_mode() {
        ...
        .expect("rename properties options should deserialize");
        let result = obfuscate(
            "const object = {foo: 1}; const {foo} = object; console.log(foo);",
            options,
        )
        .expect("obfuscation should succeed");

        assert!(
            result.code.contains("{'_0x0':foo}=object"),
            "{}",
            result.code
        );
    ...
    #[test]
    fn obfuscate_renames_class_methods_in_unsafe_property_mode() {
        let options: Options = serde_json::from_value(json!({
            "compact": true,
            "identifierNamesGenerator": "hexadecimal",
            "propertyBracketing": false,
        ...
        .expect("rename properties options should deserialize");
        let result = obfuscate(
            "class Foo { bar() { return 1; } } const foo = new Foo(); console.log(foo.bar());",
            options,
        )
        .expect("obfuscation should succeed");

        assert!(
            result.code.contains("['_0x0'](){return 0x1;}"),
            "{}",
            result.code
        );
        assert!(result.code.contains("foo._0x0()"), "{}", result.code);
    ...
    fn obfuscate_uses_root_function_string_array_wrappers_when_enabled() {
        let result = obfuscate(
            ...
            Options {
                ...
                string_array_threshold: Some(1.0),
                string_array_wrappers_count: Some(2),
                string_array_wrappers_parameters_max_count: Some(3),
                string_array_wrappers_type: Some(crate::options::StringArrayWrappersType::Function),
                rename_globals: Some(false),
                property_bracketing: Some(false),
                unicode_escape_sequence: Some(false),
                ..Options::default()
        ...
        assert!(
            result.code.contains(
                "function _0x2(index,key,unused0){return _0x1(index-0x1,key);}function _0x3(index,key,unused0){return _0x1(index-0x2,key);}"
            ),
            "{}",
            result.code
        );
        assert!(
            result.code.contains(
                "console.log([_0x2(0x1,0x2,0x3),_0x3(0x3,0x4,0x5),_0x2(0x3,0x4,0x5)].join('|'));"
            ),
            "{}",
            result.code
        );
