...
pub fn transform_class_fields(program: &mut Program, reserved_names: &[String]) {
    program.visit_mut_with(&mut ClassFieldTransform {
        reserved_name_patterns: compile_patterns(reserved_names),
    });
}

struct ClassFieldTransform {
    reserved_name_patterns: Vec<Regex>,
}

impl VisitMut for ClassFieldTransform {
    fn visit_mut_class_member(&mut self, class_member: &mut ClassMember) {
        ...
        match class_member {
            ClassMember::Method(class_method) => {
                transform_property_name(&mut class_method.key, &self.reserved_name_patterns);
            }
            ClassMember::ClassProp(class_property) => {
                transform_property_name(&mut class_property.key, &self.reserved_name_patterns);
            }
            ClassMember::AutoAccessor(auto_accessor) => {
                if let Key::Public(property_name) = &mut auto_accessor.key {
                    transform_property_name(property_name, &self.reserved_name_patterns);
                }
            }
            _ => {}
        }
...
fn transform_property_name(property_name: &mut PropName, reserved_name_patterns: &[Regex]) {
    let Some(name) = get_public_property_name(property_name) else {
        return;
    };

    if should_ignore_name(&name, reserved_name_patterns) {
        return;
    }
...
fn should_ignore_name(name: &str, reserved_name_patterns: &[Regex]) -> bool {
    name == "constructor"
...
mod tests {
    ...
    #[test]
    fn keeps_reserved_names_regex_method_key() {
        let reserved_names = vec!["^bar$".to_string()];
        let code = transform("class Foo { bar() {} baz() {} }", &reserved_names);

        assert!(code.contains("bar(){}"), "{code}");
        assert!(code.contains("['baz'](){}"), "{code}");
    }
}
